...
class MusicsDownloadModel {
  ...
  String replaceSpecialChar(String name){return name.replaceAll(RegExp(r'[,.<>?!|/\*":]'), '');}
  ...
  Future<String?> downloadMusic(
    ...
    try {
      ...
      StreamManifest streamManifest = await youtubeExplode.videos.streamsClient.getManifest(musicURL.url);
      AudioOnlyStreamInfo streamInfo = streamManifest.audioOnly.first;

      String title = replaceSpecialChar(video.title);
      Stream<List<int>> audioStream = youtubeExplode.videos.streamsClient.get(streamInfo);
      ...
      for(Directory saveDir in saveDirs){// 파일 복붙 로직 변경 가능
  ...
  Future<bool> registerMusic(Downloadable downloadable, String musicFileTitle) async {
  ...
  Iterable<String?> ExtractURL(String text) {
    RegExp urlPattern = RegExp("https:\/\/www\.youtube\.com\/watch\\?v=...........");
